...
public class lessons42587 {
    ...
    public static int solution(String[][] clothes) {
        ...
        for(int i=0; i<clothes.length; i++) {
            if(clothesHashMap.containsKey(clothes[i][1])){
                int count  = clothesHashMap.get(clothes[i][1]);
                count++;
                clothesHashMap.put(clothes[i][1], count);
            } else {
                clothesHashMap.put(clothes[i][1],1);
            }
        }

        Iterator iterator = clothesHashMap.keySet().iterator();
        answer = 1;
        while (iterator.hasNext()) {
          answer *= clothesHashMap.get(iterator.next()) + 1;
        }
        answer--;
        return answer;
    }
