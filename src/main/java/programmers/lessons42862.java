...
public class lessons42862 {
    public static void main(String[] args) {
        int[] input1 = {2,4};
        int[] input2 = {1,3,5};
        solution(5,input1,input2);
    }
    public static int solution(int n, int[] lost, int[] reserve) {

        Hashtable<Integer, Integer> clothes = new Hashtable<>();
        ...
        for(int i=1; i<=clothes.size(); i++){
            if(clothes.get(i) == 0){
                if(i-1 >=1 && clothes.get(i-1) > 1){
                    clothes.put(i, 1);
                    clothes.put(i-1, 1);
                    answer++;
                    continue;
                }
                if(i+1 <= clothes.size() && clothes.get(i+1) > 1){
                    clothes.put(i, 1);
                    clothes.put(i+1, 1);
                    answer++;
                    continue;
