package programmers;
//뉴스 클러스터링
//        여러 언론사에서 쏟아지는 뉴스, 특히 속보성 뉴스를 보면 비슷비슷한 제목의 기사가 많아
//        정작 필요한 기사를 찾기가 어렵다. Daum 뉴스의 개발 업무를 맡게 된 신입사원 튜브는
//        사용자들이 편리하게 다양한 뉴스를 찾아볼 수 있도록 문제점을 개선하는 업무를 맡게 되었다.
//
//        개발의 방향을 잡기 위해 튜브는 우선 최근 화제가 되고 있는
//        "카카오 신입 개발자 공채" 관련 기사를 검색해보았다.
//
//        카카오 첫 공채..'블라인드' 방식 채용
...
//        기사의 제목을 기준으로 "블라인드 전형"에 주목하는 기사와
//        "코딩 테스트"에 주목하는 기사로 나뉘는 걸 발견했다.
//        튜브는 이들을 각각 묶어서 보여주면 카카오 공채 관련 기사를 찾아보는 사용자에게 유용할 듯싶었다.
//
//        유사한 기사를 묶는 기준을 정하기 위해서 논문과 자료를 조사하던
//        튜브는 "자카드 유사도"라는 방법을 찾아냈다.
//
...
//        이 다중집합의 교집합 A ∩ B는 원소 "1"을 min(3, 5)인 3개, 합집합 A ∪ B는 원소 "1"을
//        max(3, 5)인 5개 가지게 된다. 다중집합 A = {1, 1, 2, 2, 3}, 다중집합 B = {1, 2, 2, 4, 5}라고 하면,
...
//        그 글자 쌍을 버린다. 예를 들어 "ab+"가 입력으로 들어오면,
